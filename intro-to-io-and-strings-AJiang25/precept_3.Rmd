...
df |>
  separate_wider_regex(
    ...
    patterns = c(
      ...
      name = "[A-Za-z]+", # Match one or more alphabets (upper or lower case)
      and create a new column 'name' with the matched value.
      ">-", # Match the literal string '>-'.
      gender = ".", # Match any single character (except newline) and
      create a new column 'gender' with the matched value.
      "_", # Match the literal character '_'.
      age = "[0-9]+" # Match one or more digits and create a new column
      'age' with the matched value.
    )
  )
```
...
my_data <- read_tsv("places_in_princeton.tsv")
head(my_data)
...
Matches one or more digits, then matches a hyphen, matches one or more digits, 
then matches a hyphen, matches one or more digits, and matches any whitespace.

For example, this will match "123-123-123 " in "123-123-123 ABC"
...
Now, extract the street address from the address string and 
search each comment for the word "Famous" to determine which locations are famous in Princeton. 
Follow this up by searching for "historic" 
Make sure that your search is case-insensitive and captures when historic is a substring as well, like "historical".

```{r, eval=FALSE}

streetnumber <- str_extract(my_data$address, "^[^,]+")
streetnumber

famous <- str_extract(my_data$comment, "[Ff]amous\\s")
famous

historic <- str_extract(my_data$comment,"[hH]istoric[\\w]*\\b")
historic
```
...
```{r, eval=FALSE}
streetnumber <- str_extract(my_data$address, "^\\d+")
streetnumber

awords <- str_match_all(my_data$comment,
"\\s[Aa][\\w]*\\b|^[Aa][\\w]*\\b")
awords
```
...
#most vowels
print(name[name$vowels == max(name$vowels),])

#highest proportion of vowels
print(name[(name$vowels/(name$vowels+name$consonants)) 
== max(name$vowels/(name$vowels+name$consonants)),])
```

Most vowels: Mariadelrosario & Mariaguadalupe have 8 vowels each
Highest proportion: Ai, Aia, Aoi, Ea, Eua, Ia, Ii, Io,...
...
data.frame(date_strings) |>
  separate_wider_regex(
    date_strings,
    patterns = c(
      year = "^\\d+", # Matches the years
      "-",
      months = "\\d+", # Matches the months
      "-",
      day = "\\d+" # Matches the days
    )
)
df
```
