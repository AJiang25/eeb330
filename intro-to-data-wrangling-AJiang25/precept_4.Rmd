...
delayed_flights <- flights %>% 
    # filters the data by departure delays greater than 12 hours
    filter(dep_delay > 720) %>%
    group_by(origin) %>% # groups the flights by origin
    count(origin) %>% # counts the number of observations
    arrange(desc(n)) # sorts the data by descending order
...
jfk_to_lax <- flights %>% 
    ...
    summarise(
      count = n(),
      average_air_time = mean(air_time, na.rm = TRUE)/60
    )
...
- **Expected Output:** 
   ...
   2. The name of at least one airport with a missing time zone and the method used to identify it.

```{r}
# data.frame that lists the time zones by frequency in descending order
airport_flights <- inner_join(airports, flights, by = c("faa" = "dest")) %>%
    group_by(tz) %>% 
    count(tz) %>% 
    arrange(desc(n))

# For time zone names: tzone could be used instead
airport_flights

# Method that returns an airport with a missing time zone
airport_flights2 <- left_join(flights, airports, by = c("dest" = "faa")) %>%
    group_by(tz) %>% 
    filter(is.na(tz)) %>%
    head(1) # truncates the entries to just one airport code

airport_flights2$dest
```

### Exercise 4: More Wrangling

- **Task:** Identify the top 3 months with the highest average departure delays in the flights dataset. For these months, calculate and present the average, minimum, and maximum departure delay.
